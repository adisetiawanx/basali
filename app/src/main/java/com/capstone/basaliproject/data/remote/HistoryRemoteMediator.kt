package com.capstone.basaliproject.data.remote
...
import com.capstone.basaliproject.data.database.RemoteKeys
...
class HistoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            val response = apiService.getAksara(page, state.config.pageSize)

            val sortedData = response.body()?.data?.sortedBy { it.scannedAt } ?: emptyList()

            val endOfPaginationReached = sortedData.isEmpty()
            ...
            database.withTransaction {
                ...
                val keys = sortedData.map {
                    RemoteKeys(id = it.predictionId, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                database.historyDao().insertHistory(sortedData)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, DataItem>): RemoteKeys? {
        return state.pages.lastOrNull {
            ...
            database.remoteKeysDao().getRemoteKeysId(data.predictionId)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, DataItem>): RemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.predictionId?.let { id ->
